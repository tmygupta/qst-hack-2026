...
def single_rho_fx(r, theta, phi, m0, m1):
    ...
    # Contract
    # Note: m0 and m1 are passed in as pre-built objects
    circ = Circuit(input_state) >> interferometer >> Circuit([m0, m1])
    sout = circ.contract().normalize()
    
    # Return the raw density matrix array
    return sout.dm().ansatz.array

# 3. HELPER: BATCH WIGNER (NumPy Side)
# -----------------------------------------------------------------------------
def compute_wigner_batch_numpy(rho_batch, xvec, pvec):
    ...
    """
    imgs = []
    # We loop in Python. This is safe because rho_batch is standard NumPy data now.
...
def generate_data(params, pix_res=32, range_val=4.0):
    ...
    # Grid Setup (Standard NumPy is fine here since we use it in the Python loop)
    xvec = np.linspace(-range_val, range_val, pix_res)
    pvec = np.linspace(-range_val, range_val, pix_res)

    # Convert Inputs to JAX for the circuit simulator
    ...
    print("Method: JAX Circuit Simulation -> NumPy Wigner Calculation")
    ...
    for n_vals in tqdm(unique_ns):
        ...
        # 2. Create Measurement Objects
        # We assume cutoff = n + 1 (minimal required to represent Fock state |n>)
        # If your physics requires higher cutoff for better fidelity, increase this (e.g. +2)
        m0 = Number((0), n=current_n_pair[0]).dual
        m1 = Number((1), n=current_n_pair[1]).dual

        # 3. Compile JAX Scanner for this group
        # We use lax.map to batch-process the circuits
        def scan_body(packed_args):
            return single_rho_fx(packed_args[0], packed_args[1], packed_args[2], m0, m1)

        # JIT the circuit simulator
        jit_scanner = jax.jit(lambda r, t, p: jax.lax.map(scan_body, (r, t, p)))

        # 4. EXECUTE JAX (Get Density Matrices)
        # This runs on GPU/Accelerator
        batch_rhos_jax = jit_scanner(r_all[indices], theta_all[indices], phi_all[indices])
        
        # 5. TRANSFER TO NUMPY
        # Critical Step: Move data out of JAX world to use standard MrMustard tools
        batch_rhos_numpy = np.array(batch_rhos_jax)
        
        # 6. COMPUTE WIGNER (Standard MrMustard)
